...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.inp = inp
        self.oup = oup
        self.conv = nn.Sequential( # expand ratio : the ratio about data how to be enlarged
            ConvBNActivation(inp, inp*expand_ratio, kernel_size=(1, 1), stride=1), #conv once , batch once , PW step
            ConvBNActivation(inp*expand_ratio, inp*expand_ratio, kernel_size=(3, 3), stride=stride, groups=inp*expand_ratio),#DW step ; single channel 3*3 conv
            nn.Conv2d(inp*expand_ratio, oup, kernel_size=(1, 1), stride=(1, 1), bias=False),#press back
            nn.BatchNorm2d(oup, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True) # normalize helps ReLU keep its performance
        )
        #so , here is the module intgerted
    def forward(self, x):
        if self.stride == 1 and self.inp == self.oup:
            return self.conv(x) + x #shortcut model , keeps the grad valid
        else:
            return self.conv(x)
    ...
    def __init__(self, wid_mul=1.0,
        ...
        if inverted_residual_setting == None:
            inverted_residual_setting = [
                #inp, oup, stride, expand_ratio | inverted residual :"enlarge"-"conv"-"press"
                [1, 16, 1, 1],
                [6, 24, 2, 2],
                [6, 32, 3, 2],
                [6, 64, 4, 2],
